...
const MarkdownIt = require('markdown-it')//預設是類別
const mdit = new MarkdownIt({ // 創建 markdown 的轉換物件
  html: true,//可以直接寫html進去
  linkify: true,//把網址自動變成超連結
  typographer: true//允許使用某個功能XD
})
  ...
  switch (op) {
    case 'view': // 顯示檔案
      if (file.indexOf('.') >= 0) { // 第一種情形：有指定副檔名，那麼直接傳回該檔案串流。
        ...
        //await:等這行指令結束再跳下一行;fs.promises.readFile:有加promises才能支援await
